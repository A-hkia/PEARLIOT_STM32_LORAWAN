...
uint8_t SX1272GetPaSelect(uint8_t power);
...
void SX1272IoInit(void)
{
  ...
  // Software controlled radio CS
  initStruct.Mode = GPIO_MODE_OUTPUT_PP;
  initStruct.Pull = GPIO_PULLUP;
  initStruct.Speed = GPIO_SPEED_HIGH;
  HW_GPIO_Init(RADIO_NSS_PORT, RADIO_NSS_PIN, &initStruct);
  HW_GPIO_Write(RADIO_NSS_PORT, RADIO_NSS_PIN, GPIO_PIN_SET);
}
...
void SX1272SetAntSwLowPower(bool status)
{
  /* power saving? set HF Switch Power Pin to input */
  if (status) {
    GPIO_InitTypeDef initStruct = { 0 };
    initStruct.Mode = GPIO_MODE_INPUT;
    initStruct.Pull = GPIO_NOPULL;
    initStruct.Speed = GPIO_SPEED_LOW;
    HW_GPIO_Init(RADIO_ANTSW_PWR_PORT, RADIO_ANTSW_PWR_PIN, &initStruct);
  }
  /* No Power Saving - set HF Switch Power Pin to ioutput and High */
  else {
    GPIO_InitTypeDef initStruct = { 0 };
    initStruct.Mode = GPIO_MODE_OUTPUT_PP;
    initStruct.Pull = GPIO_NOPULL;
    initStruct.Speed = GPIO_SPEED_LOW;
    HW_GPIO_Init(RADIO_ANTSW_PWR_PORT, RADIO_ANTSW_PWR_PIN, &initStruct);
    HW_GPIO_Write(RADIO_ANTSW_PWR_PORT, RADIO_ANTSW_PWR_PIN, GPIO_PIN_SET);
  }
